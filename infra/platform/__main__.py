...
# ---------------------------------------------------------------------------
aws_env_stack = pulumi.StackReference(f"{org_name}/aws_env/{stack}")
oidc_provider_arn = cast(str, aws_env_stack.get_output(f"{stack}-oidc-provider-arn"))

# ---------------------------------------------------------------------------
# IAM Role for Pulumi Deployments
# ---------------------------------------------------------------------------
# This role is assumed by Pulumi Deployments and ESC environments to manage
# frontend infrastructure. The trust policy allows both:
...
deployment_role = aws.iam.Role(
    f"{stack}-frontend-pulumi-oidc-deployment-role",
    name=f"{stack}-frontend-pulumi-oidc-deployment-role",
    description=(
        f"Role for Pulumi Deployments and ESC to manage frontend {stack} "
...
deployment_role_policy = aws.iam.RolePolicyAttachment(
    f"{stack}-deployment-role-admin-policy",
    role=deployment_role.name,
    policy_arn="arn:aws:iam::aws:policy/AdministratorAccess",
)
pulumi.export(f"{stack}_deployment_role_arn", deployment_role.arn)
...
# ---------------------------------------------------------------------------
# ESC Environment: AWS credentials (staging)
# ---------------------------------------------------------------------------
# Shared AWS credentials environment - provides dynamic OIDC credentials
# for frontend staging deployments.
if is_staging:
...
# ---------------------------------------------------------------------------
# Review (PR) stacks now share production's ECR repos and credentials
# rather than staging's, so this project's review-stack resources are
# owned by the production stack's `pulumi up`, not staging's.
if is_production:
    # ---------------------------------------------------------------------------
    # ESC Environment: AWS credentials (production)
    # ---------------------------------------------------------------------------
    # Shared AWS credentials environment - provides dynamic OIDC credentials
    # for frontend production deployments (including review stacks).
