...
Cypress.Commands.add("submit_pdf_file", () => {
  ...
  cy.get('[data-cy="add-document-form"] input[name=name]').type("Name of document");
  cy.get('[data-cy="add-document-form"] select[name=language_id]').select("193");
  cy.get('[data-cy="add-document-form"]  input[type="file"]').attachFile("somefile.pdf");
...
Cypress.Commands.add("get_lookups", () => {
  ...
  cy.intercept("GET", "geographies", { fixture: "geographies" }).as("getGeographies");
