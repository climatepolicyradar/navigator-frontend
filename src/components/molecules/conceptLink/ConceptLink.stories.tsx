...
const concepts: Partial<IConcept>[] = [
  {
...
export const Single: TStory = {
  args: {
    concept: concepts[0] as IConcept,
  },
};
  ...
  render: () => (
    <div className="flex flex-row gap-4">
      {concepts.map((concept) => (
        <ConceptLink key={concept.wikibase_id} concept={concept as IConcept} />
      ))}
    </div>
  ),
};
...
export const WithOnClick: TStory = {
  name: "With onClick",
  args: {
    concept: concepts[1] as IConcept,
    onClick: (concept: IConcept) => alert(concept.preferred_label),
  },
};
