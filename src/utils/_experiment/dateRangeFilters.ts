...
function normaliseDateValueForOp(op: "eq" | "not_eq" | "lt" | "lte" | "gt" | "gte", value: string): string {
  ...
  if (op === "lte" || op === "gt") return `${value}T23:59:59.999Z`;
...
export function convertDateRangeRulesToApiGroup(group: TQueryGroup): TQueryGroup {
  ...
  for (const filter of group.filters) {
    if ("field" in filter) {
      if (filter.field === "attributes.published_date" && filter.op === "between") {
        ...
        convertedFilters.push({ field: "attributes.published_date", key: "published_date", op: "lte", value: toIsoEndOfYear(parsedRange.endYear) });
        continue;
      }
      if (
        filter.field === "attributes.published_date" &&
        (filter.op === "gte" || filter.op === "lte" || filter.op === "eq" || filter.op === "not_eq" || filter.op === "lt" || filter.op === "gt")
      ) {
        convertedFilters.push({
          field: "attributes.published_date",
          key: "published_date",
  ...
  return { ...group, filters: convertedFilters };
}
...
export function convertApiDateRulesToUiGroup(group: TQueryGroup): TQueryGroup {
  ...
  let gteValue = "";
  let lteValue = "";
  group.filters.forEach((item, index) => {
  ...
  if (gteIndex !== -1 && lteIndex !== -1) {
    ...
    if (startYear !== null && endYear !== null) {
      ...
      for (let index = 0; index < group.filters.length; index += 1) {
        if (index === insertIndex) {
          withBetweenRule.push({ field: "attributes.published_date", op: "between", value: serialiseYearRange(startYear, endYear) });
        }
        if (index === gteIndex || index === lteIndex) continue;
        withBetweenRule.push(group.filters[index]);
      }
      return { ...group, filters: withBetweenRule };
    }
  }
  return group;
}
...
export function findPublishedDateBetweenValue(group: TQueryGroup): string | null {
  for (const filter of group.filters) {
    if ("field" in filter) {
      if (filter.field === "attributes.published_date" && filter.op === "between") return filter.value;
      continue;
    }
...
export function upsertPublishedDateBetweenRule(group: TQueryGroup, value: string): TQueryGroup {
  ...
  if (updated) return { ...group, filters };
  return { ...group, filters: [...group.filters, { field: "attributes.published_date", op: "between", value }] };
}
...
export function removePublishedDateRules(group: TQueryGroup): TQueryGroup {
  ...
  for (const filter of group.filters) {
    if ("field" in filter) {
      if (filter.field === "attributes.published_date") continue;
      filters.push(filter);
      continue;
    }
    const nested = removePublishedDateRules(filter);
    if (nested.filters.length > 0) filters.push(nested);
  }
